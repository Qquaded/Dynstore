...
function module.escape(s)
	return (s:gsub("[%c\"\\]", function(c)
		return "\127"..escapemap[c]
	end))
end

function module.unescape(s)
...
function module.tobase10(value)
	local n = 0
	for i = 1, #value do
...
function module.compress(text)
	local dictionary = module.copy(dictionary)
	local key, sequence, size = "", {}, #dictionary
	local width, spans, span = 1, {}, 0
	function module.listkey(key)
		local value = module.tobase93(dictionary[key])
		if #value > width then
...
function module.decompress(text)
	...
	for width = 1, #groups do
		for value in groups[width]:gmatch(('.'):rep(width)) do
			local entry = dictionary[module.tobase10(value)]
			if previous then
				if entry then
	...
	return module.unescape(table.concat(sequence))
end
return module
